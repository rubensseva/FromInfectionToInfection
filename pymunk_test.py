...
def drawPymunkCircle(pymunk_circle):
    lineThickness = 1
    radius = 10
    pygame.gfxdraw.circle(screen, int(pymunk_circle.body.position.x), int(pymunk_circle.body.position.y), radius, (255, 255, 255))
...
while not done:                 # Infinite loop simulation
    ...
    boxes = list(filter(lambda x: x.body.position.x < 800 and x.body.position.y < 800, boxes))
    circles = list(filter(lambda x: x.body.position.x < 800 and x.body.position.y < 800, circles))
    for obj in boxes:     
        drawPymunkPoly(obj)
    for obj in circles:     
    ...
    for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    new_body = pymunk.Body(1,1666)  # Create a Body with mass and moment
                    new_body.position = 400,10      # Set the position of the body
                    new_poly = pymunk.Poly.create_box(new_body) # Create a box shape and attach to body
                    space.add(new_body, new_poly)       # Add both body and shape to the simulation
